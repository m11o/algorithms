xdescribe BloomFilter do
  people = %w[Bruce\ Wayne Clark\ Kent Barry\ Allen].freeze

  let(:bloom_filter) { BloomFilter.new }

  it 'creates a new filter store with the appropriate methods' do
    # store = bloom_filter.create_store(18)
    # expect(typeof store.getValue).to eq 'function'
    # expect(typeof store.setValue).to eq 'function'
  end

  it 'hashes deterministically with all 3 hash functions' do
    str1 = 'apple'
  ...
  it 'creates an array with 3 hash values' do
    expect(bloom_filter.hash_values('abc').length).to eq 3
    expect(bloom_filter.hash_values('abc')).to eq [66, 63, 54]
  end

  it 'inserts strings correctly and return true when checking for inserted values' do
