...
describe breadthFirstSearch do
  it 'should perform BFS operation on tree' do
    ...
    expect(enterNodeCallback.mock.calls[2][0].value).to eq 'C'
    expect(enterNodeCallback.mock.calls[3][0].value).to eq 'D'
    expect(enterNodeCallback.mock.calls[4][0].value).to eq 'E'
    expect(enterNodeCallback.mock.calls[5][0].value).to eq 'F'
    expect(enterNodeCallback.mock.calls[6][0].value).to eq 'G'
    
    // Check node leaving.

    expect(leaveNodeCallback.mock.calls[0][0].value).to eq 'A'
    expect(leaveNodeCallback.mock.calls[1][0].value).to eq 'B'
    expect(leaveNodeCallback.mock.calls[2][0].value).to eq 'C'
    expect(leaveNodeCallback.mock.calls[3][0].value).to eq 'D'
    expect(leaveNodeCallback.mock.calls[4][0].value).to eq 'E'
    expect(leaveNodeCallback.mock.calls[5][0].value).to eq 'F'
    expect(leaveNodeCallback.mock.calls[6][0].value).to eq 'G'
  end
  ...
  it 'allow users to redefine node visiting logic' do
    ...
    expect(leaveNodeCallback.mock.calls[0][0].value).to eq 'A'
    expect(leaveNodeCallback.mock.calls[1][0].value).to eq 'C'
    expect(leaveNodeCallback.mock.calls[2][0].value).to eq 'F'
    expect(leaveNodeCallback.mock.calls[3][0].value).to eq 'G'
  end
